...
var DateUtilities = {
  ...
  toString: function toString(date) {
    return ""
      ...
      .concat(DateUtilities.pad((date.getMonth() + 1).toString(), 2), "-")
      .concat(DateUtilities.pad(date.getDate().toString(), 2));
  },
  ...
  isSameDayOrAfter: function isSameDayOrAfter(first, second) {
    return this.isSameDay(first, second) || this.isAfter(first, second);
  },

  dateIn: function dateIn(datesArray, date) {
    return (
      datesArray.filter(function(day) {
